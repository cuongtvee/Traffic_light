...
def matrix_similarity(MOI, p, i):
    # print("in matrix similarity")
    # print("MOI",MOI)
    # print("p",p)
    # print("i",i)
    # print("MOI.shape[0]",MOI.shape[0])
    array = np.ones(MOI.shape[0])
    for k in range(MOI.shape[0]):
        # print("MOI []",MOI[k][i:i+2])
        # print("p []",p[i:i+2])
        array[k] = cosine(MOI[k][i:i+2], p[i:i+2])
    return array
def index_line_intersect(orbi_2, orbi_3, lines):
    # print("lines",lines)
    # print("lines ++",lines[1:]+lines[:1])
    # print("satrt loop")
    # print(lines)
    # print(lines[1:]+lines[:1])
    for i, p in enumerate(zip(lines, lines[1:]+lines[:1])):
        # print(p[0],p[1], orbi_2, orbi_3)
...
def predict_direction(ROI, orbi, MOI, conf):
    if is_point_in_polygon(ROI[:-1], orbi[-2]) and not is_point_in_polygon(ROI[:-1], orbi[-1]):
        #kiem tra xem orbi va ROI giao nhau o dau
        ...
        # print("direc_pro",direc_proposed)

        moi_proposed = np.array([p[:-2] for d, p in MOI.items() if d in direc_proposed])
        # print("MOIs",MOI.items())
        # print("moi_pro",moi_proposed)
        # print("len(orbi)",len(orbi))
        ...
        return None, None
def check_intersect(line,orbi):
    return is_intersect(line[0],line[1],orbi[0],orbi[1])
...
def predict_direction_nearest_v2(MOI, direc_proposed, orbi, setofpoint):
    ...
    for p in orbi:
        ...
        for d, ps in MOI.items():
            ...
            if nearest_point in ps:
                #khong tim duoc khi diem nearest_point in ps, nhung d bot in direc_proposed
                direc_ar.append(d)
                break
