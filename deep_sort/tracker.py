...
class Tracker:
    ...
    def update(self, detections, frame_id,frame):
        def check_line(x,y):
            re = 0.011732851985559567 * x + 597.6859205776174 - y
            return True if re < 0 else False
        ...
        """
        #tim ra nhung id da match, khong duoc detect, kh duoc track
        matches, unmatched_tracks, unmatched_detections = \
            self._match(detections, frame_id)


        #update in track.py
        #Xac nhan lai xem co match khong
        for track_idx, detection_idx in matches:
            ...
            elif (detections[detection_idx].plate)!=None and len(frame)>0:
                # print((detections[detection_idx].plate))
                # print("detections[detection_idx].plate",detections[detection_idx].plate.tlwh)
                x,y,w,h=detections[detection_idx].plate.tlwh
                img_cr=frame[int(y):int(y+h),int(x):int(x+w)]
                # print(x,y,w,h)
                # print("crop shape",img_cr.shape)
                # cv2.imwrite("img/"+str(random.randint(0, 256))+".jpg",img_cr)
                self.tracks[track_idx].plates.append([img_cr,detections[detection_idx].plate.type])
        #neu khong track duoc thi thong bao co 1 fame khong track duoc
        for track_idx in unmatched_tracks:
            self.tracks[track_idx].mark_missed(self.kf, frame_id)
        #khoi tao track
        for detection_idx in unmatched_detections:
            # print("detections[detection_idx]",detections[detection_idx].tlwh)
            if check_line(detections[detection_idx].tlwh[0]+0.5*detections[detection_idx].tlwh[2],detections[detection_idx].tlwh[1]+detections[detection_idx].tlwh[3]*0.5):
